...
//Modified:
//    template: function(str, url) { return "<li>" + opt.insertText(str) + "</li>"; },
      template: function(str, url) { return "<li onclick=\"Rickets.redirect('" + url + "')\">" + opt.insertText(str) + "</li>"; },
...
//            .filter(function() { return opt.match.call(this, self.val()); })
              .filter(function() { return opt.match.call(this.text, self.val()); })
              .map(function() {
// Modification:
//              var node = $(opt.template(this))[0];
                var node = $(opt.template( this.text, this.url ))[0];
// Modification:
//              $.data(node, "originalObject", this);
                $.data(node, "originalObject", this.text );
                return node; 
              });
